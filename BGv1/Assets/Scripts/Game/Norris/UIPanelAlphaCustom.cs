...
public class UIPanelAlphaCustom : MonoBehaviour
{
	public float alpha = 1f;

	Collider[] mColliders;
	UIWidget[] mWidgets;
	List<GameObject> mInactive = new List<GameObject>();
	float[] mAlpha;
	float mLastAlpha = 1f;
	public int mLevel = 2;
	...
	void Start ()
	{
		//mColliders = GetComponentsInChildren<Collider>(false);
		mWidgets = GetComponentsInChildren<UIWidget>(false);
	...
	void UpdateAlpha ()
	{
		...
		if (mLevel == 0)
		{
			//Debug.Log (" Fade in started -- enable all game objects ");
			...
			for (int j = 0; j < mInactive.Count; j++) {
				mInactive [j].SetActive (false);
			}
		...
		{
			//Debug.Log (" Fade out started -- disable tweens and colliders ");
			// Fade out started -- disable tweens and colliders
			TweenColor[] tweens = GetComponentsInChildren<TweenColor>(false);
		...
		if (mLevel == 1)
		{
			if (alpha < 0.01f)
			{
				//Debug.Log (" Fade out finished -- disable all game objects ");
				...
				for (int i = 0, imax = trans.childCount; i < imax; ++i) {
					if (trans.GetChild (i).gameObject.activeSelf)
						...
						mInactive.Add(trans.GetChild (i).gameObject);
			...
			{
				//Debug.Log (" Fade in finished -- enable all colliders ");
